...
python src/main.py predict --method markov
...
# 与最新开奖结果比对
python src/main.py predict --check-latest
```

#### 5. 马尔可夫链分析与预测

```bash
# 运行马尔可夫链预测（默认使用最近300期数据）
...
└── src/                # 源代码目录
    ├── main.py         # 主程序
    ├── cwl_crawler.py  # 爬虫模块（300期数据）
    ├── cwl_crawler_all.py # 爬虫模块（全部历史数据）
    ├── analyzer.py     # 基础分析模块
