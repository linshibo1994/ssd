...
from utils import generate_random_numbers, generate_smart_numbers, get_latest_result, check_prize_level
from cwl_crawler import crawl_dlt_data
...
def analyze(args):
    ...
    if advanced:
        print("使用高级分析器...")
        analyzer = AdvancedAnalyzer(data_file, output_dir, periods)
        analyzer.run_advanced_analysis()
    else:
        print("使用基础分析器...")
        analyzer = BasicAnalyzer(data_file, output_dir, periods)
...
def generate(args):
    ...
    if strategy == "random":
        ...
        for i in range(count):
            front_balls, back_balls = generate_smart_numbers(df, strategy)
            print(f"[{i+1}] 前区: {front_balls}, 后区: {back_balls}")
...
def latest(args):
    ...
    # 获取最新开奖结果
    try:
        latest_result = get_latest_result(data_file)
        if latest_result:
            issue = latest_result.get("issue", "未知")
            date = latest_result.get("date", "未知")
            front_balls = latest_result.get("front_balls", [])
            back_balls = latest_result.get("back_balls", [])
...
def compare_with_latest(latest_front, latest_back):
    """将用户输入的号码与最新开奖结果进行比对
    ...
    try:
        ...
        # 判断中奖等级
        prize_level = check_prize_level(front_match, back_match)
        if prize_level > 0:
...
def markov(args):
    ...
    for i in range(count):
        front_balls, back_balls, explanation = analyzer.predict_with_markov_chain()
        print(f"\n[{i+1}] 前区: {front_balls}, 后区: {back_balls}")
    ...
    try:
        ...
        if latest_result:
            issue = latest_result.get("issue", "未知")
            date = latest_result.get("date", "未知")
            latest_front = latest_result.get("front_balls", [])
            latest_back = latest_result.get("back_balls", [])
            
            print(f"\n最新开奖结果 (期号: {issue}, 日期: {date})")
            print(f"前区号码: {latest_front}")
            print(f"后区号码: {latest_back}")
            
            # 生成一注预测号码并与最新结果比对
            front_balls, back_balls, _ = analyzer.predict_with_markov_chain()
            front_match = len(set(front_balls) & set(latest_front))
            back_match = len(set(back_balls) & set(latest_back))
            
            print(f"\n预测号码: 前区 {front_balls}, 后区 {back_balls}")
            print(f"匹配结果: 前区匹配 {front_match} 个, 后区匹配 {back_match} 个")
...
def bayesian(args):
    ...
    for i in range(count):
        front_balls, back_balls, explanation = analyzer.predict_with_bayesian()
        print(f"\n[{i+1}] 前区: {front_balls}, 后区: {back_balls}")
        print(f"预测解释: {explanation}")
    ...
    try:
        ...
        if latest_result:
            ...
            # 生成一注预测号码并与最新结果比对
            front_balls, back_balls, _ = analyzer.predict_with_bayesian()
            front_match = len(set(front_balls) & set(latest_front))
            back_match = len(set(back_balls) & set(latest_back))
...
def compare(args):
    ...
    try:
        ...
        # 创建高级分析器并进行比对分析
        analyzer = AdvancedAnalyzer(data_file, "", periods)
...
def crawl(args):
    ...
    # 爬取数据
    try:
        crawl_dlt_data(data_file, append)
        print(f"数据爬取完成，保存到 {data_file}")
    except Exception as e:
        print(f"爬取数据失败: {e}")
