...
class DLTCWLCrawler:
    ...
    def get_history_data_from_jisuapi(self, count=None):
        """从极速数据API获取大乐透历史开奖数据
        ...
        results = []
        try:
            print("从极速数据API获取大乐透历史数据...")
            import requests
            
            # 极速数据API接口地址
            api_url = "https://api.jisuapi.com/caipiao/history"
            
            # 请求参数
            params = {
                "caipiaoid": 14,  # 大乐透的ID
                "issueno": "",   # 期号，空字符串表示获取最新
                "appkey": "你的appkey"  # 需要替换为实际的appkey
            }
            ...
            if data["status"] == 0 and "result" in data:
                ...
                # 处理每一期数据
                for item in items:
                    issue = item["issueno"]  # 期号
                    date = item["date"]      # 开奖日期
                    
                    # 获取号码
                    numbers = item["number"].split()
                    if len(numbers) >= 7:  # 大乐透有5个前区号码和2个后区号码
                        front_balls = numbers[:5]
                        back_balls = numbers[5:7]
                        
                        front_balls_str = ",".join(front_balls)
                        back_balls_str = ",".join(back_balls)
                ...
                print(f"从极速数据API成功获取{len(results)}期大乐透开奖结果")
            else:
                print(f"极速数据API返回错误: {data.get('msg', '未知错误')}")
        except Exception as e:
            print(f"从极速数据API获取数据失败: {e}")
        
        return results
    
    def get_history_data_from_juhe(self, count=None):
        """从聚合数据API获取大乐透历史开奖数据
        ...
        try:
            ...
            # 请求参数
            params = {
                "key": "你的key",  # 需要替换为实际的key
                "lottery_id": "dlt"  # 大乐透的ID
            }
            ...
            if data["error_code"] == 0 and "result" in data:
                ...
                # 处理每一期数据
                for item in items:
                    issue = item["lottery_no"]  # 期号
                    date = item["lottery_date"]  # 开奖日期
                    
                    # 获取号码
                    numbers = item["lottery_res"].split("+")
                    if len(numbers) == 2:  # 大乐透格式为"前区号码+后区号码"
                        front_balls = numbers[0].split()
                        back_balls = numbers[1].split()
                ...
                print(f"从聚合数据API成功获取{len(results)}期大乐透开奖结果")
            else:
                print(f"聚合数据API返回错误: {data.get('reason', '未知错误')}")
        except Exception as e:
            print(f"从聚合数据API获取数据失败: {e}")
    ...
    def get_history_data(self, count=300):
        ...
        # 如果从中彩网获取数据失败，尝试从极速数据API获取
        if not results:
            print("从中彩网获取数据失败，尝试从极速数据API获取...")
            results = self.get_history_data_from_jisuapi(count)
        
        # 如果从极速数据API获取失败，尝试从聚合数据API获取
        if not results:
            print("从极速数据API获取数据失败，尝试从聚合数据API获取...")
            results = self.get_history_data_from_juhe(count)
        
        # 如果从所有API获取数据失败或数据不足，可以尝试从本地缓存读取
        if not results and count is not None:
            print("从所有API获取数据失败，尝试从本地缓存读取...")
            # 检查是否有本地缓存数据
