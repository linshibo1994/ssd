...
class SSQCWLCrawler:
    ...
    def save_to_csv(self, results, filename="ssq_data.csv"):
        ...
        if duplicate_count > 0:
            print(f"检测到{duplicate_count}期重复数据，已自动去除")
        print(f"去除重复后共有{len(unique_results)}期数据")
        
        file_path = os.path.join(self.data_dir, filename)
        
        try:
            # 检查是否已存在文件，如有则读取已有数据确保不重复写入
            if os.path.exists(file_path):
                ...
                try:
                    ...
                    if filtered_results:
                        print(f"将添加{len(filtered_results)}期新数据到已有文件")
                        
                        # 追加模式写入新数据
                        with open(file_path, "a", newline="", encoding="utf-8") as f:
                            writer = csv.DictWriter(f, fieldnames=["issue", "date", "red_balls", "blue_ball"])
                            if os.path.getsize(file_path) == 0:  # 如果文件为空，写入表头
                                writer.writeheader()
                            writer.writerows(filtered_results)
                        print(f"数据已追加到文件: {file_path}")
                    else:
                        print(f"没有新数据需要添加到{file_path}")
                    
                    return  # 已处理完毕，不需执行后续代码
                except Exception as e:
                    print(f"读取已有文件数据失败: {e}，将创建新文件")
