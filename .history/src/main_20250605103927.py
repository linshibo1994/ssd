    ...
    if args.command == "crawl":
        ...
        for i in range(count):
            ...
            numbers = advanced_analyzer.predict_numbers(method=method, explain=explain)
             ...
             # 格式化号码
             formatted_numbers = format_ssq_numbers(numbers)
             print(f"预测的双色球号码: {formatted_numbers}")
             
             # 获取最新开奖结果并比对
             latest_draw = get_latest_draw()
             if latest_draw:
                 latest_numbers = latest_draw["red_balls"] + [latest_draw["blue_ball"]]
                 latest_formatted = format_ssq_numbers(latest_numbers)
                 
                 # 计算中奖情况
                 prize_level = calculate_prize(numbers[:6], numbers[6], latest_numbers[:6], latest_numbers[6])
