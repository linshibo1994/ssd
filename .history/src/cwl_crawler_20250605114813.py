...
u zhou            # 使用备用URL - 500彩票网的历史数据页面
            backup_url = "https://www.500.com/ssq/history/newinc/history.php"
            ...
            while retry_count < max_retries and not success:
                try:
                    ...
                    if retry_count < max_retries:
                        ...
                        print(f"请求500彩票网主URL失败，将尝试备用URL: {e}")
            ...
            if not success:
                ...
                while retry_count < max_retries and not success:
                    try:
                        ...
                        session = requests.Session()
                        session.trust_env = False  # 禁用环境变量中的代理设置
                        response = session.get(backup_url, headers=self.cp500_headers, timeout=15)
                        # 设置正确的编码
                        response.encoding = 'gb2312'
                        html_content = response.text
                        success = True
                        ...
                        retry_count += 1
                        if retry_count < max_retries:
                            print(f"请求500彩票网备用URL失败，正在进行第{retry_count}次重试: {e}")
                            time.sleep(2 * retry_count)  # 指数退避
                        else:
                            print(f"请求500彩票网备用URL失败，已达到最大重试次数: {e}")
                            return results
