...
def main():
    ...
    if args.command == "crawl":
        ...
        advanced_analyzer = AdvancedSSQAnalyzer(data_file=get_data_file())
        if method == "all":
        ...
        for i in range(count):
            ...
            red_balls, blue_ball = advanced_analyzer.predict_numbers(method=method, explain=explain)
            print("\n预测的双色球号码:")
            print(format_ssq_numbers(red_balls, blue_ball))
            print()
            
            # 获取最新一期开奖结果进行对比
            issue, date, winning_reds, winning_blue = get_latest_draw(get_data_file())
            if issue:
                prize_level = calculate_prize(red_balls, blue_ball, winning_reds, winning_blue)
