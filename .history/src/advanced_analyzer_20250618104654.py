...
class SSQAdvancedAnalyzer:
    ...
    def load_data(self):
        ...
        try:
            ...
            # 检查必要的列是否存在
            required_columns = ['date', 'blue_ball']
            if 'red_balls' not in self.data.columns and not all(f'red_{i}' in self.data.columns for i in range(1, 7)):
                print("错误：数据文件缺少红球列")
                return False
                
            if 'blue_ball' not in self.data.columns:
                print("错误：数据文件缺少蓝球列")
                return False
            ...
            # 拆分红球列为单独的列（如果需要）
            if 'red_balls' in self.data.columns and not all(f'red_{i}' in self.data.columns for i in range(1, 7)):
            ...
            self.data['red_variance'] = self.data.apply(
                lambda x: np.var([x[f'red_{i+1}'] for i in range(6)]), axis=1
            )
            
            # 计算红球跨度（最大值-最小值）
            self.data['red_span'] = self.data.apply(
                lambda x: max([x[f'red_{i+1}'] for i in range(6)]) - 
                         min([x[f'red_{i+1}'] for i in range(6)]),
                axis=1
            )
    ...
    def predict_numbers(self, method='ensemble', explain=False):
        ...
        if method == 'stats':
            ...
            red_balls, blue_ball = self._predict_by_markov_chain(explain)
        else:  # ensemble
    ...
    def analyze_historical_correlation(self, periods_list=[5, 10, 50, 100]):
        """
        分析历史号码之间的关联性
        分析不同期数间隔的号码关联性，包括红球和蓝球的重复出现情况
        
        Args:
            periods_list: 要分析的期数间隔列表
        ...
        # 确保数据按期号排序（从新到旧）
        if 'issue_index' in self.data.columns:
            self.data = self.data.sort_values('issue_index', ascending=False).reset_index(drop=True)
        elif 'issue' in self.data.columns:
            self.data = self.data.sort_values('issue', ascending=False).reset_index(drop=True)
        else:
            # 如果没有期号相关字段，假设数据已经按从新到旧排序
            print("警告: 数据中没有期号字段，无法确保数据按期号排序")
            pass
        ...
        for periods in periods_list:
            ...
            # 遍历每一期（从最新的开始），与前N期进行比较
            for i in range(len(self.data) - periods):
                current_reds = [self.data.iloc[i][f'red_{j}'] for j in range(1, 7)]
                previous_reds = [self.data.iloc[i + periods][f'red_{j}'] for j in range(1, 7)]
                
                # 检查红球重复情况
                red_repeats = set(current_reds) & set(previous_reds)
                if red_repeats:
                    red_repeat_count += 1
                    for num in red_repeats:
                        red_repeat_numbers.append(num)
                
                # 检查蓝球重复情况
                current_blue = self.data.iloc[i]['blue_ball']
                previous_blue = self.data.iloc[i + periods]['blue_ball']
                
                if current_blue == previous_blue:
                    blue_repeat_count += 1
                    blue_repeat_numbers.append(current_blue)
            
            # 计算重复概率
            total_comparisons = len(self.data) - periods
            red_repeat_prob = red_repeat_count / total_comparisons if total_comparisons > 0 else 0
            blue_repeat_prob = blue_repeat_count / total_comparisons if total_comparisons > 0 else 0
            
            # 统计最常重复的号码
            red_counter = Counter(red_repeat_numbers)
            blue_counter = Counter(blue_repeat_numbers)
            
            most_common_red = red_counter.most_common(3) if red_counter else []
            most_common_blue = blue_counter.most_common(3) if blue_counter else []
            
            # 输出分析结果
            print(f"间隔{periods}期的红球重复概率: {red_repeat_prob:.4f}")
            print(f"间隔{periods}期的蓝球重复概率: {blue_repeat_prob:.4f}")
            
            if most_common_red:
                print("最常重复的红球号码:")
                for num, count in most_common_red:
                    print(f"  红球{num:02d}: 重复{count}次")
            
            if most_common_blue:
                print("最常重复的蓝球号码:")
                for num, count in most_common_blue:
                    print(f"  蓝球{num:02d}: 重复{count}次")
            
            # 保存结果
            results.append({
                '间隔期数': periods,
                '红球重复概率': red_repeat_prob,
                '蓝球重复概率': blue_repeat_prob,
                '最常重复红球': [num for num, _ in most_common_red[:3]],
                '最常重复蓝球': [num for num, _ in most_common_blue[:3]]
            })
        ...
        # 保存到CSV
        output_file = os.path.join(self.output_dir, 'historical_correlation.csv')
        results_df.to_csv(output_file, index=False, encoding='utf-8-sig')
        print(f"\n历史号码关联性分析结果已保存到 {output_file}")
        ...
        if len(results_df) > 0:
            ...
            plt.subplot(2, 1, 1)
            plt.plot(results_df['间隔期数'], results_df['红球重复概率'], 'ro-', label='红球重复概率')
            plt.xlabel('间隔期数')
            plt.ylabel('重复概率')
            plt.title('不同间隔期数的红球重复概率')
            plt.grid(True, linestyle='--', alpha=0.7)
            plt.legend()
            
            plt.subplot(2, 1, 2)
            plt.plot(results_df['间隔期数'], results_df['蓝球重复概率'], 'bo-', label='蓝球重复概率')
            plt.xlabel('间隔期数')
            plt.ylabel('重复概率')
            plt.title('不同间隔期数的蓝球重复概率')
            plt.grid(True, linestyle='--', alpha=0.7)
            plt.legend()
            ...
            # 保存图表
            chart_file = os.path.join(self.output_dir, 'historical_correlation_chart.png')
            plt.savefig(chart_file, dpi=300)
            plt.close()
            print(f"历史号码关联性图表已保存到 {chart_file}")
    ...
    def analyze_issue_number_correlation(self):
        """
        分析开奖号码与期号的关联性
        检查历史开奖号码是否与期号有重复出现，以及重复出现的间隔期数
        ...
        # 提取期号的数字部分
        if self.data['issue'].dtype == 'object':
            # 如果issue是字符串类型，提取数字部分
            self.data['issue_number'] = self.data['issue'].str.extract('(\d+)').astype(int)
        else:
            # 如果issue已经是数字类型，直接使用
            self.data['issue_number'] = self.data['issue']
        ...
        for i in range(1, 7):
            red_col = f'red_{i}'
            matches = self.data[self.data[red_col] == self.data['issue_number'] % 33 + 1]
            ...
            if not matches.empty:
                for _, row in matches.iterrows():
                    results.append({
                        ...
                        '期号': row['issue'],
                        '日期': row['date'],
                        '关联描述': f"红球{row[red_col]}与期号{row['issue']}的末尾数字{row['issue_number'] % 33 + 1}相同"
                    })
        
        # 分析蓝球与期号的关联
        matches = self.data[self.data['blue_ball'] == self.data['issue_number'] % 16 + 1]
        ...
        if not matches.empty:
            for _, row in matches.iterrows():
                results.append({
                    ...
                    '期号': row['issue'],
                    '日期': row['date'],
                    '关联描述': f"蓝球{row['blue_ball']}与期号{row['issue']}的末尾数字{row['issue_number'] % 16 + 1}相同"
                })
        ...
        for i, row in self.data.iterrows():
            ...
            # 检查红球是否包含期号的末尾数字
            issue_last_digits = [int(digit) for digit in str(issue_num)[-2:] if digit != '0']
            
            for digit in issue_last_digits:
                if digit <= 33 and digit in red_balls:
                    if digit not in number_appearances:
                        number_appearances[digit] = []
                    
                    number_appearances[digit].append({
                        '期号': row['issue'],
                        '日期': row['date'],
                        '类型': '红球'
                    })
            
            # 检查蓝球是否等于期号的末尾数字（对16取模）
            last_digit = issue_num % 100
            if last_digit <= 16 and last_digit == blue_ball:
                if last_digit not in number_appearances:
                    number_appearances[last_digit] = []
                
                number_appearances[last_digit].append({
                    '期号': row['issue'],
                    '日期': row['date'],
                    '类型': '蓝球'
                })
        ...
        for number, appearances in number_appearances.items():
            if len(appearances) >= 2:
                for i in range(len(appearances) - 1):
                    ...
                    previous = appearances[i + 1]
                    
                    # 计算期号差值
                    try:
                        # 尝试移除可能的'SSQ'前缀
                        current_issue = int(str(current['期号']).replace('SSQ', ''))
                        previous_issue = int(str(previous['期号']).replace('SSQ', ''))
                    except ValueError:
                        # 如果转换失败，尝试直接使用
                        current_issue = current['期号'] if isinstance(current['期号'], (int, float)) else 0
                        previous_issue = previous['期号'] if isinstance(previous['期号'], (int, float)) else 0
                    
                    issue_diff = current_issue - previous_issue
                    
                    results.append({
                        '类型': current['类型'],
                        '号码': number,
                        '期号': f"{previous['期号']} -> {current['期号']}",
                        '日期': f"{previous['日期']} -> {current['日期']}",
                        '关联描述': f"{current['类型']}{number}在期号间隔{issue_diff}期后再次与期号末尾数字匹配"
                    })
        ...
        if results:
            ...
            # 保存到CSV
            output_file = os.path.join(self.output_dir, 'issue_number_correlation.csv')
            results_df.to_csv(output_file, index=False, encoding='utf-8-sig')
            print(f"\n开奖号码与期号关联性分析结果已保存到 {output_file}")
            
            # 输出摘要
            print(f"\n发现{len(results)}个与期号相关的匹配")
            print(f"红球匹配: {len(results_df[results_df['类型'] == '红球'])}个")
            print(f"蓝球匹配: {len(results_df[results_df['类型'] == '蓝球'])}个")
    ...
    def compare_with_historical_data(self, predicted_numbers, periods=300):
        """
        将预测号码与历史数据进行对比
        分析预测号码在历史数据中的出现情况
        
        Args:
            predicted_numbers: 预测的号码，格式为(红球列表, 蓝球)
            periods: 要对比的历史期数
            
        Returns:
            对比结果的字典
        """
        red_balls, blue_ball = predicted_numbers
        
        # 限制分析的历史数据期数
        history_data = self.data.head(min(periods, len(self.data)))
        
        print(f"\n将预测号码与历史{len(history_data)}期数据进行对比...")
        print(f"预测号码: 红球 {' '.join([f'{ball:02d}' for ball in red_balls])} | 蓝球 {blue_ball:02d}")
        ...
        for i, row in history_data.iterrows():
            historical_reds = [row[f'red_{j}'] for j in range(1, 7)]
            historical_blue = row['blue_ball']
            issue = row['issue']
            date = row['date']
            
            # 计算红球匹配数
            red_matches = set(red_balls) & set(historical_reds)
            red_match_count = len(red_matches)
            
            # 检查蓝球是否匹配
            blue_match = blue_ball == historical_blue
            
            # 记录匹配情况
            if red_match_count == 6 and blue_match:
                results['完全匹配'].append({'期号': issue, '日期': date})
            
            if red_match_count == 6:
                results['红球完全匹配'].append({'期号': issue, '日期': date})
            
            if red_match_count == 5:
                results['红球匹配5个'].append({'期号': issue, '日期': date, '匹配号码': list(red_matches)})
            
            if red_match_count == 4:
                results['红球匹配4个'].append({'期号': issue, '日期': date, '匹配号码': list(red_matches)})
            
            if blue_match:
                results['蓝球匹配'].append({'期号': issue, '日期': date})
            
            # 记录与最近一期的匹配情况
            if i == 0:  # 最近一期
                results['与最近一期相同红球'] = list(red_matches)
                results['与最近一期相同蓝球'] = blue_match
        ...
        for ball in red_balls:
            ...
            for i, row in history_data.iterrows():
                historical_reds = [row[f'red_{j}'] for j in range(1, 7)]
                if ball in historical_reds:
                    count += 1
            
            results['红球号码历史出现频率'][ball] = count / len(history_data)
        
        # 统计蓝球在历史数据中的出现频率
        blue_count = history_data[history_data['blue_ball'] == blue_ball].shape[0]
        results['蓝球历史出现频率'] = blue_count / len(history_data)
        ...
        if results['完全匹配']:
            print("完全匹配的历史记录:")
            for match in results['完全匹配']:
                print(f"  {match['期号']} ({match['日期']})")
        else:
            print("在历史记录中没有完全相同的号码组合")
        
        if results['红球完全匹配']:
            print("\n红球完全匹配的历史记录:")
            for match in results['红球完全匹配']:
                print(f"  {match['期号']} ({match['日期']})")
        
        if results['红球匹配5个']:
            print("\n红球匹配5个的历史记录:")
            for match in results['红球匹配5个'][:5]:  # 最多显示5条
                print(f"  {match['期号']} ({match['日期']}), 匹配号码: {', '.join([f'{ball:02d}' for ball in match['匹配号码']])}")
        
        if len(results['红球匹配4个']) > 0:
            print(f"\n红球匹配4个的历史记录: {len(results['红球匹配4个'])}条")
        
        if results['蓝球匹配']:
            print(f"\n蓝球匹配的历史记录: {len(results['蓝球匹配'])}条")
            print(f"蓝球{blue_ball:02d}在历史数据中的出现频率: {results['蓝球历史出现频率']:.4f}")
        
        print("\n红球号码在历史数据中的出现频率:")
        for ball, freq in results['红球号码历史出现频率'].items():
            print(f"  红球{ball:02d}: {freq:.4f}")
        
        print("\n与最近一期的匹配情况:")
        if results['与最近一期相同红球']:
            print(f"  相同的红球: {', '.join([f'{ball:02d}' for ball in results['与最近一期相同红球']])}")
        else:
            print("  没有相同的红球")
        
        print(f"  蓝球是否相同: {'是' if results['与最近一期相同蓝球'] else '否'}")
        
        return results
    
    def predict_based_on_patterns(self, explain=True):
        """
        基于历史模式和规律预测双色球号码
        综合考虑历史关联性、期号关联和重复规律
        ...
        if self.data is None:
            ...
            if not success:
                print("错误：无法加载数据，请确保数据文件存在")
                return [1, 2, 3, 4, 5, 6], 1, "数据加载失败"
        
        # 首先分析历史关联性
        correlation_df = self.analyze_historical_correlation([5, 10, 20, 30, 50, 100])
        
        # 分析期号关联性
        issue_correlation_df = self.analyze_issue_number_correlation()
        
        # 获取最近几期的数据
        recent_data = self.data.head(10)  # 最近10期
        latest_data = self.data.iloc[0]   # 最新一期
        ...
        if explain:
            print("\n基于历史模式和规律的预测:")
            print(f"最新一期({latest_issue})开奖号码: 红球 {' '.join([f'{ball:02d}' for ball in latest_reds])} | 蓝球 {latest_blue:02d}")
        
        # 根据历史关联性选择可能重复的红球
        potential_repeat_reds = set()
        ...
        if not correlation_df.empty:
            for _, row in correlation_df.iterrows():
                ...
                if period < len(self.data):
                    # 获取对应间隔期数的历史红球
                    historical_reds = [self.data.iloc[period][f'red_{i}'] for i in range(1, 7)]
                    # 根据重复概率决定是否选择这些红球
                    if random.random() < row['红球重复概率'] * 1.5:  # 增加一些权重
                        potential_repeat_reds.update(historical_reds)
        
        # 从最近几期中选择一些红球
        recent_reds = set()
        for i in range(min(5, len(recent_data))):
            recent_reds.update([recent_data.iloc[i][f'red_{j}'] for j in range(1, 7)])
        
        # 根据期号关联性选择可能的红球
        issue_related_reds = set()
        # 处理不同类型的期号
        if isinstance(latest_issue, str):
            next_issue = int(latest_issue.replace('SSQ', '')) + 1
        else:
            # 如果是数字类型，直接加1
            next_issue = int(latest_issue) + 1
        next_issue_digits = [int(digit) for digit in str(next_issue) if digit != '0']
        
        for digit in next_issue_digits:
            if 1 <= digit <= 33:
                issue_related_reds.add(digit)
        
        # 综合以上因素选择红球
        selected_reds = list(potential_repeat_reds)[:2]  # 最多选择2个重复的红球
        
        # 从最近的红球中选择1-2个
        if recent_reds and len(selected_reds) < 6:
            recent_selection = random.sample(list(recent_reds), min(2, len(recent_reds), 6-len(selected_reds)))
            for ball in recent_selection:
                if ball not in selected_reds:
                    selected_reds.append(ball)
        
        # 从期号相关的红球中选择0-1个
        if issue_related_reds and len(selected_reds) < 6:
            issue_selection = random.sample(list(issue_related_reds), min(1, len(issue_related_reds), 6-len(selected_reds)))
            for ball in issue_selection:
                if ball not in selected_reds:
                    selected_reds.append(ball)
        ...
        if not correlation_df.empty:
            for _, row in correlation_df.iterrows():
                ...
                if period < len(self.data):
                    # 获取对应间隔期数的历史蓝球
                    historical_blue = self.data.iloc[period]['blue_ball']
                    # 根据重复概率决定是否选择这个蓝球
                    if random.random() < row['蓝球重复概率'] * 1.5:  # 增加一些权重
                        potential_blues.add(historical_blue)
        
        # 考虑期号相关的蓝球
        for digit in next_issue_digits:
            if 1 <= digit <= 16:
                potential_blues.add(digit)
        
        # 如果没有潜在的蓝球，从最近几期中选择
        if not potential_blues:
            for i in range(min(3, len(recent_data))):
                potential_blues.add(recent_data.iloc[i]['blue_ball'])
        
        # 如果仍然没有，随机选择
        if not potential_blues:
            selected_blue = random.randint(1, 16)
        else:
            selected_blue = random.choice(list(potential_blues))
        
        if explain:
            print("\n预测解释:")
            print("1. 红球选择:")
            print(f"   - 基于历史关联性选择的红球: {[ball for ball in selected_reds if ball in potential_repeat_reds]}")
            print(f"   - 基于最近开奖结果选择的红球: {[ball for ball in selected_reds if ball in recent_reds and ball not in potential_repeat_reds]}")
            print(f"   - 基于期号关联选择的红球: {[ball for ball in selected_reds if ball in issue_related_reds and ball not in potential_repeat_reds and ball not in recent_reds]}")
            print(f"   - 随机补充的红球: {[ball for ball in selected_reds if ball not in potential_repeat_reds and ball not in recent_reds and ball not in issue_related_reds]}")
            
            print("2. 蓝球选择:")
            if selected_blue in potential_blues:
                if selected_blue in [recent_data.iloc[i]['blue_ball'] for i in range(min(3, len(recent_data)))]:
                    print(f"   - 蓝球{selected_blue:02d}来自最近几期的开奖结果")
                else:
                    print(f"   - 蓝球{selected_blue:02d}基于历史关联性或期号关联选择")
            else:
                print(f"   - 蓝球{selected_blue:02d}是随机选择的")
        
        # 构建解释文本
        explanation = ""
        if explain:
            explanation = "基于历史模式和规律的预测，结合了历史关联性、最近开奖结果和期号关联性"
        
        return selected_reds, selected_blue, explanation
    ...
    def _visualize_markov_chain(self, results):
        ...
        try:
            ...
            # 添加节点和边
            for current, nexts in blue_probs.items():
                for next_ball, prob in nexts.items():
                    if prob > 0.1:  # 只显示概率大于0.1的转移
                        G.add_edge(f'B{current}', f'B{next_ball}', weight=prob, label=f'{prob:.2f}')
            ...
            weights = [data['weight'] * 3 for _, _, data in edges]  # 根据权重调整边的宽度
            ...
            plt.title('蓝球马尔可夫链转移概率网络图')
            plt.axis('off')
            
            # 保存图表
            chart_file = os.path.join(self.output_dir, 'blue_ball_markov_chain.png')
            plt.savefig(chart_file, dpi=300, bbox_inches='tight')
            plt.close()
            print(f"蓝球马尔可夫链网络图已保存到 {chart_file}")
        except Exception as e:
            print(f"可视化蓝球马尔可夫链时出错: {e}")
        ...
        try:
            ...
            for pos in range(1, 7):
                red_probs = results['红球转移概率'][pos]
                
                # 创建转移矩阵
                matrix = np.zeros((33, 33))
                for i in range(1, 34):
                    for j in range(1, 34):
                        if i in red_probs and j in red_probs[i]:
                            matrix[i-1, j-1] = red_probs[i][j]
                
                # 绘制热力图
                plt.subplot(2, 3, pos)
                plt.imshow(matrix, cmap='hot', interpolation='nearest')
                plt.colorbar(label='转移概率')
                plt.title(f'红球位置{pos}的转移概率热力图')
                plt.xlabel('下一期号码')
                plt.ylabel('当前期号码')
            ...
            # 保存图表
            chart_file = os.path.join(self.output_dir, 'red_ball_markov_chain.png')
            plt.savefig(chart_file, dpi=300, bbox_inches='tight')
            plt.close()
            print(f"红球马尔可夫链热力图已保存到 {chart_file}")
        except Exception as e:
    ...
    def _predict_by_markov_chain(self, explain=False):
        ...
        predicted_reds = []
        for pos in range(1, 7):
            current_ball = latest_data[f'red_{pos}']
            pos_transitions = self._markov_results['红球转移概率'][pos]
            
            # 如果当前球有转移概率记录
            if current_ball in pos_transitions and pos_transitions[current_ball]:
                # 根据转移概率选择下一个球
                candidates = list(pos_transitions[current_ball].keys())
                probabilities = list(pos_transitions[current_ball].values())
                next_ball = random.choices(candidates, weights=probabilities, k=1)[0]
            else:
                # 如果没有转移记录，随机选择
                next_ball = random.randint(1, 33)
            
            # 确保不重复
            while next_ball in predicted_reds:
                next_ball = random.randint(1, 33)
        ...
        # 预测蓝球
        blue_transitions = self._markov_results['蓝球转移概率']
        ...
        if latest_blue in blue_transitions and blue_transitions[latest_blue]:
            ...
            predicted_blue = random.choices(candidates, weights=probabilities, k=1)[0]
        else:
            # 如果没有转移记录，随机选择
        ...
        if explain:
            print("\n预测解释:")
            print("1. 红球预测:")
            for i, (current, predicted) in enumerate(zip([latest_data[f'red_{j}'] for j in range(1, 7)], predicted_reds)):
                pos = i + 1
                if current in self._markov_results['红球转移概率'][pos] and predicted in self._markov_results['红球转移概率'][pos][current]:
                    prob = self._markov_results['红球转移概率'][pos][current][predicted]
                    print(f"   - 红球位置{pos}: {current:02d} -> {predicted:02d} (转移概率: {prob:.4f})")
                else:
                    print(f"   - 红球位置{pos}: {current:02d} -> {predicted:02d} (随机选择)")
            
            print("\n2. 蓝球预测:")
            if latest_blue in blue_transitions and predicted_blue in blue_transitions[latest_blue]:
                prob = blue_transitions[latest_blue][predicted_blue]
                print(f"   - 蓝球: {latest_blue:02d} -> {predicted_blue:02d} (转移概率: {prob:.4f})")
            else:
                print(f"   - 蓝球: {latest_blue:02d} -> {predicted_blue:02d} (随机选择)")
        
        # 构建解释文本
        explanation = "基于马尔可夫链状态转移概率的预测，分析了历史号码的转移规律"
        
        return predicted_reds, predicted_blue, explanation
