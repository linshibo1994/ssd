    ...
    def get_history_data(self, count=300):
        ...
        try:
            ...
            for batch in range(num_batches):
                ...
                print(f"API响应内容: {response.text[:500]}...")  # 只打印前500个字符，避免输出过长
                
                # 解析JSON数据
                data = response.json()
                
                # 打印解析后的数据结构
                print(f"解析后的数据结构: {json.dumps(data, ensure_ascii=False)[:500]}...")
                
                if "result" in data:
                    result_data = data["result"]
                    print(f"结果数据类型: {type(result_data)}")
                    print(f"结果数据长度: {len(result_data) if isinstance(result_data, list) else '非列表类型'}")
                    
                    if isinstance(result_data, list) and len(result_data) > 0:
                        print(f"第一条结果示例: {json.dumps(result_data[0], ensure_ascii=False)}")
                
                if data.get("state") == 1 and "result" in data and isinstance(data["result"], list):
                    # 提取开奖结果
